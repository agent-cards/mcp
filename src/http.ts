...
import { JWT_SECRET, MCP_BASE_URL } from './config.js';
...
app.use(mcpAuthRouter({
  ...
  serviceDocumentationUrl: new URL('https://agentcard.sh'),
...
app.get('/.well-known/mcp/server-card.json', (_req, res) => {
  ...
  res.json({
    ...
    tools: [
      ...
      { name: 'x402_fetch', description: 'HTTP fetch with automatic payment on 402 responses' },
    ...
    keywords: ['mcp', 'virtual-card', 'ai-agent', 'payments', 'visa', 'fintech', 'x402'],
    logo: 'https://agentcard.sh/card.png',
  });
});
