...
app.get('/.well-known/mcp/server-card.json', (_req, res) => {
  ...
  res.json({
    ...
    tools: [
      ...
      { name: 'get_funding_status', description: 'Poll card creation status after checkout payment' },
...
// Bind to a port + attach the Chrome-extension WebSocket bridge only when this
// file is the process entry (local dev `pnpm dev:http` or Docker `node http.js`).
// On Vercel the entry is api/[[...slug]].ts which just re-exports the app —
// the listen() and ws upgrade handler are skipped because Vercel runs each
// request as a stateless function (no long-lived server, no WS support).
const isMainEntry =
