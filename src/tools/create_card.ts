...
export const createCardDefinition = {
  name: 'create_card',
  description:
    'Create a new virtual debit card. Requires a saved payment method (call setup_payment_method first). Your payment method is only charged when the card is used. Cards expire after 7 days if unused. Limits: max $50 per card, max 5 cards per month (free plan). Upgrade to Basic for higher limits. Use sandbox: true for testing.',
  inputSchema: {
  ...
  if (res.status === 422) {
    ...
    if (body.status === 'user_info_required' || body.status === 'phone_terms_required') {
      const text = [
        ...
        '  2. Acceptance of the Stripe Issuing cardholder terms of service',
        '',
